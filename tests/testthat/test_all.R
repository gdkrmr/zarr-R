...
for(data_type in data_types) {
  ...
  test_that(
    "zarr_dataset created a directory on disk",
    expect_true(dir.exists(path))
  )
  test_that(
    "zarr_dataset is the correct s3 class",
    expect_s3_class(data_set, "zarr_dataset")
  )
  test_that(
    "the path from zarr_dataset object is correct",
    expect_equal(get_path(data_set), path)
  )
  test_that(
    "zarr_dataset has the correct dimensions",
  ...
  data_attr2 <- zarr::read_attributes(data_set)
  ...
  test_that(
    "attribute shape is correct",
    expect_equal(data_attr$shape, as.list(shape))
  )
  test_that(
    "attribute chunking is correct",
  ...
  test_that(
    "`[` with the wrong number of parameters fails",
  ...
  test_that("range_to_offset_shape", {
    expect_equal(
      zarr:::range_to_offset_shape(1:3, 1:3, 1:3),
      list(offset = c(0, 0, 0), shape = c(3, 3, 3))
    )
    expect_equal(
      zarr:::range_to_offset_shape(2:4, 2:4, 2:4),
      list(offset = c(1, 1, 1), shape = c(3, 3, 3))
    )
    expect_equal(
      zarr:::range_to_offset_shape(1, 2:5, 1),
